...
export class BookListComponent implements OnInit {
  ...
  fetchBooks(): void {
    this.http.get<Book[]>('http://localhost:5088/api/book').subscribe({
      next: (data) => {
        this.books = data;
  ...
  addBook(): void {
    this.http.post<Book>('http://localhost:5088/api/book', this.newBook).subscribe({
      next: (createdBook) => {
  ...
  saveBook(): void {
    if (this.editingBook) {
      this.http.put<Book>(`http://localhost:5088/api/book/${this.editingBook.id}`, this.editingBook).subscribe({
        next: (updatedBook) => {
  ...
  deleteBook(id: number): void {
    ...
    if (confirmed) {
      this.http.delete(`http://localhost:5088/api/book/${id}`).subscribe({
        next: () => {
